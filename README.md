...
2. We need to implement Trie structure. We will create it as class so we can also write our methods.
    1. We need to define class and write method prototypes in a header file. In our case it is ```aho_corasick.h```:
       ```
    ...
    2. Now we need to implement **```void add_word(const std::string &word);```**, **```void build_failure_links();```** and **```void search(const std::string &text);```** methods:
       1. **```void add_word(const std::string &word)```** method: <br>
        this method will add search words and create the Trie:
            ...
            ```
        2. **```void build_failure_links()```** method: <br>
           ...
           ```
        3. **```void search(const std::string &text)```** method: <br>
